...
function initOpenClose() {
	...
	jQuery('ul.accordion>li').openClose({
		...
		animEnd: function(state, self){
			...
			if(self.linkFlag && state){
				if(jQuery(window).width() > 1023){
					var top = self.holder.offset().top - header.outerHeight(true);
					ScrollTo(top, animDuration)
				}else{
					var top = self.holder.offset().top;
					ScrollTo(top, animDuration)
				}
			}
		},
		animStart: function(state, self){
			...
			if(!state){
				self.linkFlag = false;
				if(self.holder.data('link')){
					self.holder.data('link').removeClass(activeClass);
				}
			}else{
				if(self.holder.data('link')){
					self.holder.data('link').addClass(activeClass);
				}
			}
		}
	});
	...
	jQuery('.table').each(function(){
		var holder = jQuery(this),
			openCloses = jQuery('ul.accordion>li'),
			links = holder.find('.scroll-link');

		links.each(function(){
			var link = jQuery(this),
				openClose = jQuery(link.attr('href'));
		...
		links.on('click', function(e){
			...
			if(!link.hasClass(activeClass)){
				if(openCloses.filter('.active').length){
					openCloses.filter('.active').data('OpenClose').hideSlide();
				}
				link.data('openClose').data('OpenClose').linkFlag = true;
				link.data('openClose').data('OpenClose').showSlide();
			}else{
				if(jQuery(window).width() > 1023){
					var top = link.data('openClose').offset().top - header.outerHeight(true);
					ScrollTo(top, animDuration)
				}else{
					var top = link.data('openClose').offset().top
					ScrollTo(top, animDuration)
				}
			}
		});
...
function initLightbox() {
	jQuery('a.lightbox-opener, a[rel*="lightbox"]').each(function(){
		var link = jQuery(this);
		link.fancybox({
			padding: 10,
			margin: 0,
...
function initChildClasses() {
	jQuery('.t-col, .t-col a').children(':nth-child(2n+1)').addClass('nth-child');
	jQuery('.t-col a').children(':last-child').addClass('last-child');
}
		...
		attachEvents: function() {
			...
			self.outsideClickHandler = function(e) {
				if(self.options.hideOnClickOutside) {
					var target = $(e.target);
					if (!target.is(self.holder) && !target.closest(self.holder).length && !target.closest('.scroll-link').length) {
						self.hideSlide();
					}
				}
			};
